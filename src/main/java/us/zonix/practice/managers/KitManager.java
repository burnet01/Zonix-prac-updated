...
public class KitManager
{
    ...
    private void loadKits() {
        ...
        kitSection.getKeys(false).forEach(name -> {
            final ItemStack[] contents = ((List<ItemStack>)configurationSection.get(name + ".contents")).toArray(new ItemStack[0]);
            final ItemStack[] armor = ((List<ItemStack>)configurationSection.get(name + ".armor")).toArray(new ItemStack[0]);
            final ItemStack[] kitEditContents = ((List<ItemStack>)configurationSection.get(name + ".kitEditContents")).toArray(new ItemStack[0]);
            final List<String> excludedArenas = configurationSection.getStringList(name + ".excludedArenas");
            final List<String> arenaWhiteList = configurationSection.getStringList(name + ".arenaWhitelist");
            final ItemStack icon = (ItemStack)configurationSection.get(name + ".icon");
            final boolean enabled = configurationSection.getBoolean(name + ".enabled");
            final boolean ranked = configurationSection.getBoolean(name + ".ranked");
            final boolean combo = configurationSection.getBoolean(name + ".combo");
            final boolean sumo = configurationSection.getBoolean(name + ".sumo");
            final boolean build = configurationSection.getBoolean(name + ".build");
            final boolean spleef = configurationSection.getBoolean(name + ".spleef");
            final boolean parkour = configurationSection.getBoolean(name + ".parkour");
            final boolean hcteams = configurationSection.getBoolean(name + ".hcteams");
            final boolean bestOfThree = configurationSection.getBoolean(name + ".bestOfThree");
            final boolean premium = configurationSection.getBoolean(name + ".premium");
            final int priority = configurationSection.getInt(name + ".priority");
            final Optional<KnockbackProfile> knockback = Optional.ofNullable(KnockbackConfig.getKbProfileByName(configurationSection.getString(name + ".knockback")));
