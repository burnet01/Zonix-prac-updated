...
public class InventorySnapshot
{
    ...
    public InventorySnapshot(final Player player, final Match match) {
        ...
        int potCount = 0;
        if (potionMatch) {
            potCount = Arrays.stream(contents).filter(Objects::nonNull).mapToInt(ItemStack::getDurability).filter(d -> d == 16421).count();
            ...
            final int soupCount = Arrays.stream(contents).filter(Objects::nonNull).map(ItemStack::getType).filter(d -> d == Material.MUSHROOM_SOUP).count();
